...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score','money','invitecode','leader_id','relation','level','coldmoney','rebatemoney'];
     ...
     * @param string $password 密码
     * @param string $mobile   手机号
     * @param string $recUid   推荐人id
     * @param array  $extend   扩展参数
     * @return boolean
     */
    public function webRegister($mobile, $password, $nickname, $recUid, $extend = [])
    {
        if ($mobile && User::getByMobile($mobile)) {
            $this->setError('Mobile already exist');
            return false;
        ...
        $data = [
            ...
            'level'    => 0,
            'score'    => 0,
            'avatar'   => '/uploads/logo01.png',
            'leader_id'=>$recUid?$recUid:0
        ];
        $invitecode = db('user')->order('id desc')->value('invitecode');
        if($invitecode){
            $data['invitecode'] = $invitecode+1;
        }
        $relation = db('user')->where('id',$recUid)->value('relation');
        $data['relation'] = $recUid.'-'.$relation;
        $params = array_merge($data, [
            'nickname'  => $nickname,
            'salt'      => Random::alnum(),
            'jointime'  => $time,
        ...
        try {
            ...
            $this->_token = Random::uuid();
            Token::set($this->_token, $user->id, $this->keeptime);
            Message::create(['user_id'=>$recUid,'type'=>'reg','content'=>'直推下级'.$mobile.'注册成功']);
            $projectConfig = new ProjectConfig();
            $levelConfig = ProjectConfig::getConfig('leveljson');
            $levelConfig = json_decode($levelConfig[0],true);
            db('user')->where('id',$user->id)->setInc('score',$levelConfig[0]['num']);
            ScoreLog::create(['user_id' => $user->id, 'score' => '+'.$levelConfig[0]['num'], 'before' => 0, 'after' => $levelConfig[0]['num'], 'memo' => '领取青铜会员奖励']);
            $projectConfig->userRebate($recUid,$user->id,'rec');
            //注册成功的事件
            Hook::listen("user_register_successed", $this->_user, $data);
            Db::commit();
        } catch (Exception $e) {
